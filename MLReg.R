### multiple linear regression on load curves ###
...
wds <- maply(1:nrow(DT2), function(n) lubridate::wday(as.Date(unlist(DT2[n,1]))))
wks <- maply(1:nrow(DT2), function(n) lubridate::week(as.Date(unlist(DT2[n,1]))))

DT3 <- bind_cols(DT2, data.frame(wds), data.frame(wks))
...
lm_m_1 <- lm(Load.24 ~ 0 + ., data = matrix_train)
...
plot(DT3$`24`, type = 'l', lwd = 2)
lines(lm_m_1$fitted.values, type = 'l', lwd = 2, col = 'red')

mean(lm_m_1$residuals)
var(lm_m_1$residuals)
...
lm_m_2 <- lm(Load.24 ~ 0 + Load.23 + wds + wks + wds:wks, data = matrix_train)
summary(lm_m_2)

plot(DT3$`24`, type = 'l', lwd = 2)
lines(lm_m_2$fitted.values, type = 'l', lwd = 2, col = 'skyblue')

mean(lm_m_2$residuals)
var(lm_m_2$residuals)
